...
class PluginNagiosqlDisplay extends CommonDBTM {
   ...
   function showBoard($width='', $limit='') {
      ...
      $where = '';
      if ($limit == 'hosts') {
         $where = "`plugin_nagiosql_services_id`='0' ";
      } else if ($limit == 'services') {
         $where = "`plugin_nagiosql_services_id`>0 ";
      }      
      if (isset($_SESSION['plugin_nagiosql']['service']['field'])) {
         foreach ($_SESSION['plugin_nagiosql']['service']['field'] as $key=>$value) {
            $wheretmp = '';
            if (isset($_SESSION['plugin_nagiosql']['service']['link'][$key])) {
               $wheretmp.= " ".$_SESSION['plugin_nagiosql']['service']['link'][$key]." ";
            }

            $wheretmp .= Search::addWhere(
                                   "",
                                   0,
                                   "PluginNagiosqlService",
                                   $_SESSION['plugin_nagiosql']['service']['field'][$key],
                                   $_SESSION['plugin_nagiosql']['service']['searchtype'][$key],
                                   $_SESSION['plugin_nagiosql']['service']['contains'][$key]);
            if (!strstr($wheretmp, "``.``")) {
               if ($where != ''
                       AND !isset($_SESSION['plugin_nagiosql']['service']['link'][$key])) {
                  $where .= " AND ";
               }
               $where .= $wheretmp;
            }
      ...
      if ($where != '') {
         $where = " WHERE ".$where;
         $where = str_replace("`".getTableForItemType("PluginNagiosqlDisplay")."`.", 
                 "", $where);
         
      }
      $query = "SELECT * FROM `".getTableForItemType("PluginNagiosqlService")."` ".$where."
         ORDER BY `plugin_nagiosql_services_id`";
      ...
      printPager($_GET['start'], $numrows, $CFG_GLPI['root_doc']."/plugins/nagiosql/front/display.php", $parameters);
      ...
      while ($data=$DB->fetch_array($result)) {
         if ($data['plugin_nagiosql_services_id'] == '0') {
   ...
   static function displayLine($data) {
      ...
      $pNagiosqlService = new PluginNagiosqlService();
      $pNagiosqlServiceH = new PluginNagiosqlService();
      
      $pNagiosqlService->getFromDB($data['id']);
      
      echo "<td width='32' class='center'>";
      $shortstate = self::getState($data['state'], $data['state_type']);
      echo "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_".$shortstate."_32.png'/>";
      echo "</td>";
      if (isset($pNagiosqlService->fields['itemtype']) 
              AND $pNagiosqlService->fields['itemtype'] != '') {

         $itemtypemat = $pNagiosqlService->fields['itemtype'];
         $itemmat = new $itemtypemat();
         $itemmat->getFromDB($pNagiosqlService->fields['items_id']);
         echo "<td>";
      ...
      if ($pNagiosqlService->fields['plugin_nagiosql_services_id'] == '0') {
         echo "<td>".$itemmat->getLink(1)."</td>";
      } else {
         $pNagiosqlServiceH->getFromDB($pNagiosqlService->fields['plugin_nagiosql_services_id']);
         $itemtypemat = $pNagiosqlServiceH->fields['itemtype'];
         $itemmat = new $itemtypemat();
         $itemmat->getFromDB($pNagiosqlServiceH->fields['items_id']);
         echo "<td>".$pNagiosqlService->getLink(1).$nameitem." ".$LANG['networking'][25]." ".$itemmat->getLink(1)."</td>";
      }
      ...
      echo "<td class='center'>";
      $to = new PluginNagiosqlRrdtool();
      $plu = new PluginNagiosqlServiceevent();
      $img = '';

      if ($to->displayGLPIGraph("PluginNagiosqlService", $data['id'], "12h")) {
         $img = "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/front/send.php?file=PluginNagiosqlService-".$data['id']."-12h.gif'/>";
         echo "<a href='".$CFG_GLPI['root_doc']."/plugins/nagiosql/front/display.form.php?itemtype=PluginNagiosqlService&items_id=".$data['id']."'>";
      } else {
         $img = '';
      }
      if ($img != '') {
         showToolTip($img, array('img'=>$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/stats_32.png"));
         echo "</a>";
      }
      echo "</td>";
      ...
      if ($pNagiosqlService->fields['plugin_nagiosql_services_id'] > 0) {
         ...
         $a_serv = $pNagiosqlService->find("`plugin_nagiosql_services_id`='".$data['id']."'");
         ...
         $tooltip .= "<tr class='tab_bg_1'>
            ...
            <img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_".$shortstate."_32.png'/></td></tr>";
         foreach ($a_serv as $sdata) {
            ...
            $tooltip .= "<tr class='tab_bg_1'><td>".$sdata['name']." :</td><td>
                     <img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_".$stateserv."_32.png'/></td></tr>";
         }
         $tooltip .= "</table>";
         ...
         if ($globalserv_state['red'] > 0) {
            $img = $CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_red_32.png";
         } else if ($globalserv_state['red_soft'] > 0) {
            $img = $CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_red_32_soft.png";
         } else if ($globalserv_state['orange'] > 0) {
            $img = $CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_orange_32.png";
         } else if ($globalserv_state['orange_soft'] > 0) {
            $img = $CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_orange_32_soft.png";
         } else if ($globalserv_state['green'] > 0) {
            $img = $CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_green_32.png";
         } else if ($globalserv_state['green_soft'] > 0) {
            $img = $CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_green_32_soft.png";
         }
   ...
   function displayGraphs($itemtype, $items_id) {
      ...
      echo "</tr>";
         ...
         echo "<td align='center'>";
         $img = '';
         if ($itemtype == 'PluginNagiosqlService') {
            $plu->parseToRrdtool($items_id, $itemtype);
            $to->displayGLPIGraph($itemtype, $items_id, $time, 900);
            $img = "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/front/send.php?file=".$itemtype."-".$items_id."-".$time.".gif'/>";
         } else {
            $plu->parseToRrdtool($items_id, $itemtype);
            $to->displayGLPIGraph($itemtype, $items_id, $time, 900);
            $img = "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/front/send.php?file=".$itemtype."-".$items_id."-".$time.".gif'/>";
   ...
   function displayCounters() {
      ...
      $ok = countElementsInTable("glpi_plugin_nagiosql_services", 
              "(`state`='OK' OR `state`='UP') AND `state_type`='HARD'");
      
      $warning = countElementsInTable("glpi_plugin_nagiosql_services", 
              "(`state`='WARNING' OR `state`='UNKNOWN' OR `state`='RECOVERY' OR `state`='FLAPPING' OR `state` IS NULL)
                 AND `state_type`='HARD'");
      
      $critical = countElementsInTable("glpi_plugin_nagiosql_services", 
              "(`state`='DOWN' OR `state`='UNREACHABLE' OR `state`='CRITICAL' OR `state`='DOWNTIME')
                 AND `state_type`='HARD'");
    
      $warning_soft = countElementsInTable("glpi_plugin_nagiosql_services", 
              "(`state`='WARNING' OR `state`='UNKNOWN' OR `state`='RECOVERY' OR `state`='FLAPPING' OR `state` IS NULL)
                 AND `state_type`='SOFT'");
      
      $critical_soft = countElementsInTable("glpi_plugin_nagiosql_services", 
              "(`state`='DOWN' OR `state`='UNREACHABLE' OR `state`='CRITICAL' OR `state`='DOWNTIME')
                 AND `state_type`='SOFT'");
      
      $ok_soft = countElementsInTable("glpi_plugin_nagiosql_services", 
      ...
      echo "<td>";
         ...
         echo "<th width='70'>";
         if ($critical > 0) {
            echo "<a href='".$CFG_GLPI['root_doc']."/plugins/nagiosql/front/display.php?".
               "field[0]=3&searchtype[0]=contains&contains[0]=CRITICAL".
                  ...
                  "&itemtype=PluginNagiosqlService&start=0&glpi_tab=2'>
               <img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_red_40.png'/>
                  </a>";
         }
         echo "</th>";
         echo "<th width='70'>";
         if ($warning > 0) {
            echo "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_orange_40.png'/>";
         }
         echo "</th>";
         echo "<th width='70'>";
         echo "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_green_40.png'/>";
         echo "</th>";
         echo "</tr>";
      ...
      echo "<td>";
         ...
         if ($critical_soft > 0) {
            echo "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_red_40_soft.png'/>";
         }
         echo "</th>";
         echo "<th width='70'>";
         if ($warning_soft > 0) {
            echo "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_orange_40_soft.png'/>";
         }
         echo "</th>";
         echo "<th width='70'>";
         if ($ok_soft > 0) {
            echo "<img src='".$CFG_GLPI['root_doc']."/plugins/nagiosql/pics/box_green_40_soft.png'/>";
         }
         echo "</th>";
         echo "</tr>";
         echo "<th height='30'>";
